...
// Calculates the square of CSR matrix, as long as it's square.
 csr csrSquareAlt(csr converted, int **table, long size) {
	...
	for (long row = 0; row < size; row++) {
		...
		cilk_for (long column = 0; column < size; column++) {
			int cellValue = 0;

			cilk_for (long element = start; element < end; element++) {
...
// CSR-CSR Hadamard (element-wise) operation.
 csr newhadamard(csr csrTable, csr square, long size) {
	...
	cilk_for (int i = 0; i < size; i++) {
		...
		cilk_for (int j = csrTable.rowIndex[i]; j < csrTable.rowIndex[i+1]; j++) {
			cilk_for (int k = square.rowIndex[i]; k < square.rowIndex[i+1]; k++) {
